...
# 추가(수정)
def get_replay_with_label(generator, classifier, batchsize, n_class, n_inc, mean_vector):

  z_ = Variable(torch.rand((batchsize, z_dim)))
  if use_cuda:
    z_ = z_.to(device)
  images = generator(z_)
  # 삭제 label = classifier.predict(images)
  #print("len(images)", len(images[0]))
  #print("len(label)", len(label[0]))
#  print("type(images), type(label)", type(images), type(label))
  # 수정
  images, lbl_for_one_hot = selector(images, n_class-n_inc, k, mean_vector)
  # print(lbl_for_one_hot)
...
# 추가 함수
def get_label_mean_vectors(train_loader):
    mean_vectors = {}
    for batch in train_loader:
        x_batch, y_batch = batch
        y_labels = torch.argmax(y_batch, dim=1)  # 원-핫 인코딩 라벨에서 실제 라벨로 변환
        unique_labels = y_labels.unique()

        for label in unique_labels:
            label_mask = (y_labels == label)
            label_data = x_batch[label_mask]
            mean_vector = label_data.mean(dim=0)
            if label.item() not in mean_vectors:
                mean_vectors[label.item()] = []
            mean_vectors[label.item()].append(mean_vector)

    # 각 라벨의 평균 벡터 계산
    for label, vectors in mean_vectors.items():
        mean_vectors[label] = torch.stack(vectors).mean(dim=0)
    ...
    for n, (inputs, labels) in enumerate(train_loader):
      ...
      if task > 0 :
        ...
        # the more there are past tasks more data need to be regenerated.
          replay, re_label = get_replay_with_label(G_saved, C_saved, batchsize, n_class=n_class, n_inc=n_inc, mean_vector = mean_vector)
...
#          print("orejgia")
  ...
  # 추가 mean_vector 구하기 (형태 딕셔너리 0 : tensor([-0.7071, -0.7071, -0.7071]))
  # 고민되는 점... train set에서만 가져가야 하는지 replay된 ... 것도 가져가야하는지...........
  new_mean_vector = get_label_mean_vectors(train_loader)
  mean_vector.update(new_mean_vector)
