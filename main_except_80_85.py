...
for i in range(80, 85):
  delete_elem = np.where(Y_train == i)
  X_train = np.delete(X_train, delete_elem, axis = 0)
  Y_train = np.delete(Y_train, delete_elem, axis = 0)

  delete_elem_test = np.where(Y_test == i)
  X_test = np.delete(X_test, delete_elem_test, axis = 0)
  Y_test = np.delete(Y_test, delete_elem_test, axis = 0)

Y_train_ = copy.deepcopy(Y_train)
Y_test_ = copy.deepcopy(Y_test)

for l in range(85, 100):
  Y_train[np.where(Y_train_ == l)] = l-5
  Y_test[np.where(Y_test_ == l)] = l-5
