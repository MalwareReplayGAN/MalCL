...
def get_dataloader(x, y, batchsize, n_class, scaler):
    ...
    samples_weight = np.array([weight[t-n_class+5] for t in y_])
...
# 수정
def Rank(sumArr, img, class_num, k, index_):
...
#    zip(img_list, y1_list)
#    print("time for to list", time_for_to_list = time.time()-start)
    y = pandas.DataFrame({'a': sumArr, 'b':img_list, 'd':index})
#    print("time for dataframe", time_for_dataframe = time.time()-start - time_for_to_list)
    y = y.sort_values(by=['a'], axis = 0)
...
# 추가(수정)
def selector(images, n_class, k, mean_vector):
    img = []
    lbl = []
    # lbl_for_one_hot = []
    index = []
    ...
    for i in range(n_class):
        sumArr = GetL2Dist(images, mean_vector[i])
        new_images, new_label, new_index = Rank(sumArr, images, i, k, index)
        img = img + new_images
        lbl = lbl + new_label
        index = index + [new_index]
    duplicate_count = duplicate(index)
